...
const PLATFORM_COLORS: Record<string, { bg: string; color: string; icon: string }> = {
  "Reddit": { bg: "oklch(0.60 0.20 28 / 0.1)", color: "oklch(0.55 0.18 145)", icon: "🔴" },
    ...
    <div style={{ maxWidth: 1100 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 24 }}>
        {[
          ...
          { label: "Reddit", value: "8", color: "oklch(0.55 0.18 145)" },
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {["sell fast", "motivated seller", "divorce", "foreclosure", "behind on payments", "can't afford", "must sell", "price reduced", "need to sell", "insurance spike"].map(kw => (
                <span key={kw} style={{ padding: "4px 8px", background: "oklch(0.60 0.20 28 / 0.1)", border: "1px solid oklch(0.55 0.18 145 / 0.2)", borderRadius: 4, fontSize: 11, color: "oklch(0.55 0.18 145)", fontFamily: "'JetBrains Mono', monospace" }}>{kw}</span>
              ))}
            </div>
          </div>
          <div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {platforms.map(p => (
          <button key={p} onClick={() => setActiveFilter(p)} style={{
            ...
            background: activeFilter === p ? "oklch(0.55 0.18 145)" : "transparent",
            borderColor: activeFilter === p ? "oklch(0.55 0.18 145)" : "oklch(0.22 0.015 30)",
              ...
              <div style={{ display: "flex", gap: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                    ...
                    {sig.keywords.map(kw => (
                      <span key={kw} style={{ padding: "2px 8px", background: "oklch(0.60 0.20 28 / 0.1)", border: "1px solid oklch(0.55 0.18 145 / 0.2)", borderRadius: 4, fontSize: 11, color: "oklch(0.55 0.18 145)" }}>{kw}</span>
                    ))}
                  </div>
